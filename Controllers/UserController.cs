...
namespace VipcoMachine.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpGet("GetUserByEmpCode")]
        public async Task<IActionResult> GetUserByEmpCode(string EmpCode)
        {
            if (!string.IsNullOrEmpty(EmpCode))
            {
                var HasUser = await this.repositoryUser.GetFirstOrDefaultAsync(z => z, z => z.EmpCode == EmpCode);
                return new JsonResult(HasUser, this.DefaultJsonSettings);
            }
            return NoContent();
        }
