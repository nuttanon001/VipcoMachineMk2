...
namespace VipcoMachine.Controllers
{
    ...
    public class JobCardMasterController : Controller
    {
        ...
        private readonly IRepository<JobCardMaster> repository;
        private readonly IRepository<JobCardMasterHasAttach> repositoryHasAttach;
        private readonly IRepository<JobCardDetail> repositoryDetail;
        private readonly IRepository<UnitsMeasure> repositoryUom;
        private readonly IRepository<CuttingPlan> repositoryCut;
        private readonly IRepository<AttachFile> repositoryAtt;
        private readonly IRepository<ProjectCodeDetail> repositoryProDetail;
        private readonly IRepository<TypeMachine> repositoryTypeMachine;
                    ...
                    if (JobMaster != null)
                    {
                        var CuttingPlans = this.repositoryCut.GetAllAsQueryable()
                                                    .Where(x => x.ProjectCodeDetailId == JobMaster.ProjectCodeDetailId &&
                                                                !x.JobCardDetails.Any() && x.TypeCuttingPlan == TypeCuttingPlan.CuttingPlan)
                                                    .AsQueryable();
        ...
        public async Task<IActionResult> CheckCuttingPlanWaiting()
        {
            ...
            try
            {
                var hasCuttingPlanWaiting = await this.repositoryCut.GetAllAsQueryable()
                                                      .Where(x => !x.JobCardDetails.Any() && x.TypeCuttingPlan == TypeCuttingPlan.CuttingPlan)
                                                      .Include(x => x.JobCardDetails)
                                                      .AsNoTracking()
                                                      .ToListAsync();
        ...
        public async Task<IActionResult> PostAttac(int JobCardMasterId, string CreateBy, IEnumerable<IFormFile> input2)
        {
            string Message = "";
            try
            {
